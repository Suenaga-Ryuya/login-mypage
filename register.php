...
<body>
    ...
    <form action="register_confirm.php" method="POST" enctype="multipart/form-data">
      <div class="form_contents">
        ...
        <div class="mail">
          ...
          <input type="text" class="formbox" size="40" name="mail" pattern="^[a-z0-9_.%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$" required>
        </div>

        <div class="password">
          <div class="hissu">必須</div><label>パスワード（半角英数字6文字以上）</label><br>
          <input type="password" id="password" class="formbox" size="40" name="password" pattern="^[a-zA-Z0-9]{6,}$" required>
        </div>

        <div class="password">
          <div class="hissu">必須</div><label>パスワード確認</label><br>
          <input type="password" class="formbox" size="40" name="comfirm_password" id="comfirm" oninput="ConfirmPassword(this)" required>
        </div>
  ...
  <script>
    function ComfirmPassword(comfirm) {
      var input1 = password.value;
      var input2 = comfirm.value;

      if (input1 !== input2) {
        comfirm.setCustomValidity("パスワードが合致しません");
      }else {
